...
# Status constants to avoid fragile string matching
STATUS_RUNNING = "running"
STATUS_SUCCESS = "success"
STATUS_FAILED = "failed"
STATUS_TIMEOUT = "timeout"
VALID_STATUSES = {STATUS_RUNNING, STATUS_SUCCESS, STATUS_FAILED, STATUS_TIMEOUT}
...
class WorkflowMetrics:
    ...
    def is_terminal(self) -> bool:
        ...
        return self.status in {STATUS_SUCCESS, STATUS_FAILED, STATUS_TIMEOUT}
...
class MetricsCollector:
    ...
    def __init__(self, retention_hours: int = 24):
        ...
        # Use RLock to prevent deadlock when sync code calls async-capable methods
    ...
    def start_cleanup_task(self) -> None:
        ...
        # Use get_running_loop() only; do not fall back to get_event_loop()
    ...
    def _cleanup_old_metrics(self) -> None:
        ...
        with self._lock:
            to_remove = [
                run_id
                for run_id, wf in self._workflows.items()
    ...
    def track_workflow(self, workflow_name: str, run_id: str):
        ...
        try:
            ...
            self._record_histogram(
                "cray_workflow_duration_seconds",
                metrics.duration,
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for key, value in counters.items():
            if "cray_workflow_total" not in key:
                continue
            total_runs += value
            # Parse labels from key instead of fragile string matching
            # Key format: cray_workflow_total{label1="val1",label2="val2"}
            label_part = key.split("{", 1)[-1].split("}")[0] if "{" in key else ""
            for part in label_part.split(","):
                part = part.strip()
                if 'status="success"' in part:
                    successful += value
                elif 'status="failed"' in part:
                    failed += value
                elif 'status="running"' in part:
                    running += value

        # Fallback to workflow dict if no counters
        if total_runs == 0:
            total_runs = len(workflows)
            successful = sum(1 for w in workflows if w.status == STATUS_SUCCESS)
            failed = sum(1 for w in workflows if w.status == STATUS_FAILED)
            running = sum(1 for w in workflows if w.status == STATUS_RUNNING)
    ...
    def get_prometheus_metrics(self) -> str:
        ...
        for key, values in histograms.items():
            if "workflow_duration" in key and values:
                ...
                # Use DEFAULT_BUCKETS for Prometheus histogram
